          ...
          {key, {{:enum, type}, fields}}, acc ->
            {_index, hash} =
              Enum.reduce(
                fields,
                {0, %{}},
                fn field_dec, {index, hash_acc} ->
                  {field, s_index} =
                    case field_dec do
                      field when is_atom(field) -> {field, index}
                      {field, s_index} -> {field, s_index}
                    end

                  hash_acc = Map.put(hash_acc, field, s_index) |> Map.put(s_index, field)
                  {s_index + 1, hash_acc}
                end
              )
